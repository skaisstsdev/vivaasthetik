...
import { Calendar, Clock, LogOut, Check, X, Trash2, Edit2, Plus, ArrowLeft } from 'lucide-react';
...
function CalendarTab({ db, locale }: { db: ReturnType<typeof useDatabase>, locale: string }) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 animate-in fade-in duration-300">
      <div className="bg-white p-6 md:p-8 border border-gray-100 rounded-sm shadow-sm flex-shrink-0 mx-auto lg:mx-0 w-full lg:w-auto">
        <DayPicker 
          mode="single"
          ...
          modifiersClassNames={{
            ...
            hasBooking: "font-bold text-blue-600 relative after:content-[''] after:absolute after:bottom-1 after:left-1/2 after:-translate-x-1/2 after:w-1 after:h-1 after:bg-blue-600 after:rounded-full"
          }}
        />
      </div>
      ...
      <div className="flex-1 bg-white border border-gray-100 rounded-sm shadow-sm overflow-hidden flex flex-col min-h-[400px]">
        <div className="p-6 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
          <div>
            <h3 className="font-medium text-xl text-gray-900">{format(selectedDate, 'd MMMM yyyy', { locale: ru })}</h3>
            <p className="text-sm text-gray-500 mt-1">
              {dayBookings.length} {dayBookings.length === 1 ? 'запись' : 'записей'} на этот день
            </p>
          </div>
          <button 
            onClick={() => setShowManualModal(true)}
            className="flex items-center gap-2 bg-gray-900 text-white px-4 py-2 rounded-sm text-sm uppercase tracking-widest hover:bg-gray-800 transition-colors"
          >
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        ...
        <div className="flex bg-white border border-gray-200 rounded-sm overflow-hidden text-sm font-mono uppercase tracking-widest">
          {[
            { id: 'all', label: 'Все' },
            ...
            <button 
              ...
              className={`px-4 py-2 border-r last:border-r-0 border-gray-200 transition-colors ${filter === f.id ? 'bg-gray-900 text-white' : 'hover:bg-gray-50'}`}
            >
              {f.label}
            </button>
          ))}
    ...
    <div className="flex flex-col gap-8 animate-in fade-in duration-300">
      <div>
        ...
        <p className="text-gray-500 mb-8 max-w-2xl">
          Настройте стандартные рабочие дни недели. Календарь автоматически сгенерирует слоты с шагом 1 час между Временем начала и конца.
        </p>
      </div>
      
      <div className="bg-white border border-gray-100 rounded-sm overflow-hidden shadow-sm max-w-3xl">
            ...
            <div key={day.id} className={`flex flex-col md:flex-row md:items-center justify-between p-6 ${index !== daysOfWeek.length - 1 ? 'border-b border-gray-100' : ''}`}>
              ...
              <div className={`flex items-center gap-4 transition-opacity ${config?.isWorking ? 'opacity-100' : 'opacity-30 pointer-events-none'}`}>
                <div className="flex flex-col gap-1">
                  <label className="text-[10px] text-gray-400 uppercase tracking-widest">Начало</label>
                  <select 
                    value={config?.startTime || '10:00'}
                    onChange={e => db.updateWorkingHours(day.id, { ...config, startTime: e.target.value })}
                    className="border border-gray-200 p-2 text-sm outline-none focus:border-gray-900 bg-transparent rounded-sm"
                  >
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-[10px] text-gray-400 uppercase tracking-widest">Конец</label>
                  <select 
  ...
  return (
    <div className="flex flex-col gap-12 animate-in fade-in duration-300">
      ...
      <section className="bg-white border border-gray-100 rounded-sm shadow-sm p-6 md:p-8">
        <h2 className="text-2xl font-light mb-2">Закрытые периоды (Отпуск)</h2>
        <p className="text-gray-500 mb-6 text-sm max-w-3xl">Укажите период (От и До). В эти дни запись будет полностью недоступна.</p>
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-end mb-8">
          <div className="flex flex-col gap-1 w-full sm:w-auto">
            ...
            <input type="date" value={newVacationStart} onChange={e => setNewVacationStart(e.target.value)} className="border border-gray-200 p-2 rounded-sm" />
          </div>
          <div className="flex flex-col gap-1 w-full sm:w-auto">
            <label className="text-[10px] text-gray-400 uppercase tracking-widest">По дату</label>
            <input type="date" value={newVacationEnd} onChange={e => setNewVacationEnd(e.target.value)} className="border border-gray-200 p-2 rounded-sm" />
          </div>
          <button 
            onClick={() => {
            ...
            className="bg-gray-900 text-white px-6 py-2 rounded-sm text-sm uppercase tracking-widest hover:bg-gray-800 disabled:opacity-50 h-[42px] w-full sm:w-auto"
          >
            Добавить
          </button>
        </div>
        ...
        {db.blockedPeriods.length > 0 && (
          <div className="grid gap-2">
            {db.blockedPeriods.map(p => (
              <div key={p.id} className="flex justify-between items-center p-4 bg-gray-50 border border-gray-100 rounded-sm">
      ...
      {/* SECTION: DAY EXCEPTIONS */}
      <section className="bg-white border border-gray-100 rounded-sm shadow-sm p-6 md:p-8">
        <h2 className="text-2xl font-light mb-2">Исключения для конкретных дней</h2>
        <p className="text-gray-500 mb-6 text-sm max-w-3xl">Здесь можно переопределить настройки для одного конкретного дня. Например, сделать выходной день рабочим, или заблокировать обеденные часы.</p>
        ...
        <div className="flex flex-col gap-2 mb-8 max-w-xs">
          ...
          <input type="date" value={exceptionDate} onChange={e => setExceptionDate(e.target.value)} className="border border-gray-200 p-3 rounded-sm outline-none focus:border-gray-900" />
        </div>

        {exceptionDate && (
          <div className="border-t border-gray-100 pt-6 animate-in fade-in">
            <h3 className="font-medium text-lg mb-4">Настройки для {format(parseISO(exceptionDate), 'd MMMM yyyy', { locale: ru })}</h3>
            
            <div className="flex flex-col gap-6">
              {/* Working override */}
              <div className="flex items-center gap-4">
                <div 
                  className={`w-12 h-6 rounded-full p-1 cursor-pointer transition-colors ${db.dateExceptions[exceptionDate]?.isWorking !== false ? 'bg-green-500' : 'bg-red-500'}`}
                  onClick={() => {
                    const current = db.dateExceptions[exceptionDate] || {};
                    db.updateDateException(exceptionDate, { ...current, isWorking: current.isWorking === false ? true : false });
                  }}
                >
                  <div className={`w-4 h-4 bg-white rounded-full transition-transform ${db.dateExceptions[exceptionDate]?.isWorking !== false ? 'translate-x-6' : 'translate-x-0'}`} />
                </div>
                <span className="font-medium">{db.dateExceptions[exceptionDate]?.isWorking !== false ? 'Рабочий день' : 'Выходной день (Закрыт)'}</span>
              </div>

              {db.dateExceptions[exceptionDate]?.isWorking !== false && (
                <div className="p-6 bg-gray-50 border border-gray-100 rounded-sm">
                  <h4 className="font-medium text-sm mb-4 uppercase tracking-widest text-gray-500">Заблокировать часы</h4>
                  <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-8 gap-2">
                    {Array.from({length: 13}).map((_, i) => {
                      const time = `${(i + 8).toString().padStart(2, '0')}:00`;
                      const currentBlocks = db.dateExceptions[exceptionDate]?.blockedHours || [];
                      const isBlocked = currentBlocks.includes(time);
                      
                      return (
                        <button
                          key={time}
                          onClick={() => {
                            const newBlocks = isBlocked ? currentBlocks.filter(t => t !== time) : [...currentBlocks, time];
                            db.updateDateException(exceptionDate, { ...db.dateExceptions[exceptionDate], blockedHours: newBlocks });
                          }}
                          className={`py-2 px-1 border rounded-sm text-sm transition-colors ${
                            isBlocked 
                  ...
                  <p className="text-xs text-gray-400 mt-4">Зачеркнутые часы будут недоступны для записи.</p>
                </div>
              )}
            </div>
          </div>
...
function BookingCard({ booking, db, locale, showDate = false }: { booking: any, db: ReturnType<typeof useDatabase>, locale: string, showDate?: boolean }) {
  ...
  const serviceName = servicesData.find(s => s.slug === booking.serviceSlug)?.title[locale as 'de'|'ru'] || booking.serviceSlug;
  ...
  if (isCancelling) {
    return (
      <div className="bg-red-50 border border-red-100 p-6 flex flex-col items-center justify-center gap-4 animate-in fade-in">
        <h4 className="text-red-800 font-medium text-lg">Уверены, что хотите отменить эту запись?</h4>
        <p className="text-red-600 text-sm">{booking.clientName} — {serviceName}</p>
        <div className="flex gap-4 mt-2">
          <button onClick={() => setIsCancelling(false)} className="px-6 py-2 border border-red-200 text-red-700 bg-white hover:bg-red-50 rounded-sm text-sm uppercase tracking-widest">Нет, назад</button>
          <button onClick={() => { db.updateBookingStatus(booking.id, 'cancelled'); setIsCancelling(false); }} className="px-6 py-2 bg-red-600 text-white hover:bg-red-700 rounded-sm text-sm uppercase tracking-widest">Да, отменить</button>
        </div>
      </div>
    );
  }
  ...
  if (isRescheduling) {
    ...
    return (
      <div className="bg-gray-50 border border-gray-200 p-6 flex flex-col gap-6 animate-in fade-in">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <DayPicker 
            ...
            className="bg-white p-4 border border-gray-100 rounded-sm font-sans"
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-3 gap-2">
              {slots.length === 0 ? (
                <span className="text-sm text-gray-400 col-span-3">Нет времени</span>
              ) : (
                slots.map(t => (
                  <button key={t} onClick={() => setNewTime(t)} className={`py-2 text-sm border rounded-sm transition-colors ${newTime === t ? 'bg-gray-900 text-white border-gray-900' : 'bg-white border-gray-200 hover:border-gray-400'}`}>
                    {t}
                  </button>
                ))
              )}
            ...
            <button 
              ...
              className="w-full mt-6 bg-blue-600 text-white py-3 rounded-sm uppercase tracking-widest text-sm hover:bg-blue-700 disabled:opacity-50 transition-colors"
            >
              Подтвердить перенос
            </button>
          </div>
  ...
  return (
    <div className={`bg-white border border-gray-100 p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 hover:shadow-md transition-shadow relative overflow-hidden ${booking.status === 'cancelled' ? 'opacity-60 grayscale' : ''}`}>
      ...
      <div className="flex flex-col gap-1 md:w-1/3">
        ...
        <span className="text-gray-900 text-sm mt-2 font-medium bg-gray-50 p-2 rounded-sm inline-block w-fit">{serviceName}</span>
        {booking.clientNotes && <span className="text-gray-400 text-xs italic mt-2">Заметка: {booking.clientNotes}</span>}
      </div>
      ...
      <div className="flex flex-col gap-2 md:w-1/3">
        {showDate && (
          <div className="flex items-center gap-2 text-gray-900">
            ...
            <span className="font-medium">{format(parseISO(booking.date), 'EEEE, d MMMM yyyy', { locale: ru })}</span>
          </div>
        )}
        <div className="flex items-center gap-2 text-gray-900">
        ...
        <div className="mt-2 text-xs uppercase tracking-widest font-mono">
      ...
      <div className="flex items-center gap-2 md:w-1/3 md:justify-end flex-wrap">
        {booking.status === 'pending' && (
          <button onClick={() => db.updateBookingStatus(booking.id, 'confirmed')} className="p-3 bg-green-50 text-green-600 hover:bg-green-100 rounded-full transition-colors" title="Подтвердить">
            <Check className="w-5 h-5" />
          </button>
        )}
        {booking.status !== 'cancelled' && (
          <>
            <button onClick={() => setIsRescheduling(true)} className="p-3 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-full transition-colors" title="Перенести">
              <Edit2 className="w-5 h-5" />
            </button>
            <button onClick={() => setIsCancelling(true)} className="p-3 bg-red-50 text-red-600 hover:bg-red-100 rounded-full transition-colors" title="Отменить">
              <Trash2 className="w-5 h-5" />
            </button>
          </>
        )}
        {booking.status === 'cancelled' && (
          <button onClick={() => db.updateBookingStatus(booking.id, 'pending')} className="text-xs text-gray-400 hover:text-gray-900 underline underline-offset-4 uppercase tracking-widest ml-4">
            Восстановить
          </button>
        )}
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white rounded-sm shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto flex flex-col md:flex-row">
        <div className="p-8 border-b md:border-b-0 md:border-r border-gray-100 bg-gray-50 w-full md:w-1/2">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-2xl font-light">Новая запись</h3>
          ...
          <DayPicker 
            ...
            className="bg-white p-4 border border-gray-100 rounded-sm font-sans mx-auto"
            modifiersClassNames={{ selected: "bg-gray-900 text-white font-medium hover:bg-gray-800 rounded-sm", today: "font-semibold text-gray-900 bg-gray-50 rounded-sm" }}
          />
          <div className="mt-6">
            <label className="text-[10px] text-gray-400 uppercase tracking-widest mb-2 block">Доступное время</label>
            <div className="grid grid-cols-4 gap-2">
              {slots.length === 0 ? <span className="text-sm text-gray-400">Нет слотов</span> : slots.map(t => (
                <button key={t} type="button" onClick={() => setTime(t)} className={`py-2 text-sm border rounded-sm transition-colors ${time === t ? 'bg-gray-900 text-white border-gray-900' : 'bg-white border-gray-200 hover:border-gray-400'}`}>{t}</button>
              ))}
            </div>
          </div>
        </div>
        
        <form onSubmit={handleSubmit} className="p-8 w-full md:w-1/2 flex flex-col gap-6 relative">
          ...
          <div className="flex flex-col gap-2 mt-8 md:mt-0">
            <label className="text-xs text-gray-400 uppercase tracking-widest">Процедура</label>
            <select value={serviceSlug} onChange={e => setServiceSlug(e.target.value)} className="border-b border-gray-300 py-2 outline-none focus:border-gray-900 bg-transparent" required>
              {servicesData.map(s => <option key={s.slug} value={s.slug}>{s.title[locale as 'ru']}</option>)}
            </select>
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-xs text-gray-400 uppercase tracking-widest">Имя клиента</label>
            <input type="text" value={name} onChange={e => setName(e.target.value)} className="border-b border-gray-300 py-2 outline-none focus:border-gray-900 bg-transparent" required />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-xs text-gray-400 uppercase tracking-widest">Телефон</label>
            <input type="tel" value={phone} onChange={e => setPhone(e.target.value)} className="border-b border-gray-300 py-2 outline-none focus:border-gray-900 bg-transparent" />
          </div>
          
          <div className="mt-auto pt-8">
            <button type="submit" disabled={!date || !time || !name} className="w-full bg-gray-900 text-white py-4 uppercase tracking-widest text-sm hover:bg-gray-800 disabled:opacity-50 transition-colors">
              Добавить запись
            </button>
          </div>
        </form>
